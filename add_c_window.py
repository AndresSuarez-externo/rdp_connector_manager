...
class AddClientWindow:
    ...
    def __init__(self,master, password, callback):
        ...
        self.l_client_name = tk.Label(self.top, text="Nombre:")
        self.l_client_name.grid(row=1, column=1, sticky=tk.W)
        self.l_client_name.config(width=lab_width)

        self.l_contact = tk.Label(self.top, text="Contacto:")
    ...
    def save_client(self):
        ...
        if name:
            ...
            if status:
                ...
                messagebox.showwarning(title="Error during save process", message=message)

        else:
            messagebox.showwarning(title="No client name entered.", message="Proporcionar el nombre del cliente antes de guardar")
